...
export default function BottomNav({ variant = "employee" }: BottomNavProps) {
  ...
  return (
    <nav className="fixed bottom-4 left-1/2 z-50 w-[calc(100%-2rem)] max-w-[34rem] -translate-x-1/2 md:hidden">
      <div className="relative overflow-hidden rounded-[2rem] border border-[var(--card-border)] bg-[var(--card-bg)]/90 px-3 py-3 shadow-2xl shadow-slate-400/30 backdrop-blur-2xl">
          ...
          {employeeMenus.map((menu) => {
            ...
            return (
              <Link
                key={menu.href}
                href={menu.href}
                className={`relative flex h-[4.7rem] flex-col items-center justify-center gap-1 rounded-[1.45rem] text-xs font-black transition-all duration-300 active:scale-[0.96] ${active
                    ? "bg-[var(--primary)] text-white shadow-xl shadow-blue-900/25"
                    : "text-[var(--text-muted)] hover:bg-[var(--background)] hover:text-[var(--primary)]"
                  }`}
              >
                {active ? (
                  <span className="absolute -top-2 h-1.5 w-12 rounded-full bg-[var(--secondary)]" />
                ) : null}
