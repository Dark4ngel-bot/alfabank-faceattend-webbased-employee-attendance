...
export function AppBankSelect({
  ...
  const currentBank = BANK_OPTIONS.find(
    (b) =>
        ...
        <div className="flex items-center gap-3 min-w-0">
          {currentBank ? (
            <>
              {/* LOGO BADGE */}
              <span
                className={`flex h-7 shrink-0 items-center justify-center rounded-lg px-2.5 text-xs font-black uppercase tracking-wider shadow-sm ${currentBank.brandClassName}`}
              >
        ...
        <div className="absolute left-0 right-0 top-full z-[200] mt-2 max-h-[380px] w-full flex flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-2xl ring-1 ring-black/5 animate-in fade-in slide-in-from-top-2 duration-150">
          ...
          <div className="overflow-y-auto max-h-[250px] p-1.5 space-y-1 no-scrollbar">
            {filteredBanks.map((bank) => {
              const isSelected =
                value.trim().toLowerCase() === bank.code.toLowerCase() ||
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    {/* BANK LOGO BADGE */}
                    <span
                      className={`flex h-7 w-16 shrink-0 items-center justify-center rounded-lg text-[10px] font-black uppercase tracking-wider shadow-xs ${bank.brandClassName}`}
                    >
