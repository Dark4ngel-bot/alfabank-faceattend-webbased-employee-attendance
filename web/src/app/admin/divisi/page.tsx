...
export default function DepartmentsPage() {
  ...
  async function loadDepartments() {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || data.message || "Gagal mengambil data divisi.");
      }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const bodyPayload = editingDepartment
        ? { id: editingDepartment.id, name, status: form.status }
  ...
  async function handleDelete(item: Department) {
    const confirmed = confirm(
      `Apakah Anda yakin ingin menghapus divisi "${item.name}"?`
    );
    ...
    try {
      ...
      const response = await fetch(`/api/admin/departments?id=${item.id}`, {
        method: "DELETE",
      });
  ...
  return (
    <MobileShell variant="admin">
      ...
      <AppHeader title="Divisi Perusahaan" variant="admin" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="modal-panel relative w-full max-w-md rounded-[2.5rem] border border-white bg-white p-6 shadow-2xl md:p-8">
            ...
            <h2 className="text-2xl font-black text-slate-950">
              {editingDepartment ? "Ubah Divisi" : "Tambah Divisi"}
            </h2>
            <p className="mt-1 text-sm font-semibold text-slate-500">
              {editingDepartment
                ? "Perbarui nama atau status divisi terpilih."
                : "Masukkan nama divisi baru untuk disimpan."}
            </p>
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-5">
              <div>
                ...
                <AppInput
                  ...
                  placeholder="Contoh: IT & Technology, Operational, HRD"
                  required
                />
              </div>
