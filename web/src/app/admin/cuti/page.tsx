...
export default function AdminLeaveReportPage() {
  ...
  function openAnswerModal(
    ...
    setPendingAction({
      ...
      locationUnlockRequested: item.locationUnlockRequested,
    });
    ...
    <MobileShell variant="admin" withBottomPadding={false}>
      ...
      {pendingAction ? (
        ...
        <div className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-[2rem] bg-white p-5 shadow-2xl shadow-slate-950/30 md:p-7">
          ...
          {pendingAction.locationUnlockRequested &&
          pendingAction.status === "approved" ? (
            <label className="mt-4 flex items-start gap-3 rounded-2xl border border-blue-100 bg-[#f8fbff] p-4">
              <input
                type="checkbox"
                checked={locationUnlockApproved}
                onChange={(event) =>
                  setLocationUnlockApproved(event.target.checked)
                }
                className="mt-1 h-4 w-4 rounded border-blue-200 text-[#123c8c]"
              />
              <span className="text-sm font-semibold leading-6 text-slate-600">
                Setujui pembukaan kunci lokasi presensi untuk pengajuan ini.
              </span>
            </label>
          ) : null}
          ...
          <div className="mt-6 flex flex-col-reverse gap-3 md:flex-row md:justify-end">
            ...
            <button
              ...
              disabled={Boolean(isUpdatingId)}
              className={`inline-flex items-center justify-center gap-2 rounded-2xl px-5 py-3 text-sm font-black text-white transition active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60 ${pendingAction.status === "approved"
