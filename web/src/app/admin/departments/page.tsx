...
import { Edit, Loader2, Network, Plus, Search, Trash2, X } from "lucide-react";
...
export default function DepartmentsPage() {
  ...
  const filteredDepartments = useMemo(() => {
    ...
    return departments.filter((department) => {
      const matchesSearch =
        !keyword || department.name.toLowerCase().includes(keyword);
      const matchesStatus =
  ...
  async function loadDepartments() {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || data.message || "Gagal mengambil divisi.");
      }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || data.message || "Gagal menyimpan divisi.");
      }
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              ...
              <div className="rounded-2xl border border-blue-100 bg-[#f6f8ff] p-4">
                ...
                <p className="mt-1 text-sm leading-6 text-slate-500">
                  Divisi berdiri sendiri sebagai label. Relasi dengan kantor,
                  unit, dan jabatan tidak lagi dipakai.
                </p>
              </div>
