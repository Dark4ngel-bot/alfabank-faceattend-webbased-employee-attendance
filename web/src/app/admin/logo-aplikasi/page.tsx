...
import { CheckCircle2, ImageIcon, Loader2, RotateCcw, Upload } from "lucide-react";
...
export default function AdminSiteLogoPage() {
  ...
  const loadLogo = useCallback(async () => {
    try {
      ...
      setErrorMessage(
        error instanceof Error ? error.message : "Gagal mengambil logo aplikasi.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setErrorMessage(
        error instanceof Error ? error.message : "Gagal memperbarui logo aplikasi.",
      );
    } finally {
      setIsSaving(false);
    }
  ...
  return (
    <MobileShell variant="admin">
      ...
      <main className="min-h-[calc(100dvh-88px)] bg-[#f6f8ff] px-4 pb-24 pt-6 md:px-8 lg:px-16">
        <div className="mx-auto grid max-w-6xl gap-5 lg:grid-cols-[0.95fr_1.35fr]">
