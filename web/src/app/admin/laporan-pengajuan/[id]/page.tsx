...
const approvedAnswerOptions = [
  {
    ...
    value: "Pengajuan disetujui oleh admin.",
  },
  {
    label: "Disetujui, koordinasi pekerjaan",
    value:
      "Pengajuan disetujui. Silakan pastikan pekerjaan yang sedang berjalan sudah dikoordinasikan terlebih dahulu.",
  },
  {
    label: "Disetujui, lampirkan bukti bila diperlukan",
    value:
      "Pengajuan disetujui. Apabila diperlukan, silakan lengkapi dokumen pendukung setelah kembali bekerja.",
  },
  {
    label: "Tulis sendiri",
    value: "custom",
...
const rejectedAnswerOptions = [
  {
    ...
    value: "Pengajuan ditolak oleh admin.",
  },
  {
    label: "Ditolak karena jadwal belum memungkinkan",
    value:
      "Pengajuan belum dapat disetujui karena jadwal kerja pada tanggal tersebut belum memungkinkan.",
  },
  {
    label: "Ditolak, perlu revisi tanggal",
    value:
      "Pengajuan belum dapat disetujui. Silakan ajukan ulang dengan tanggal yang lebih sesuai.",
  },
  {
    label: "Ditolak, alasan belum cukup jelas",
    value:
      "Pengajuan belum dapat disetujui karena alasan pengajuan belum cukup jelas. Silakan ajukan ulang dengan keterangan yang lebih lengkap.",
  },
  {
    label: "Tulis sendiri",
    value: "custom",
...
export default function AdminLeaveRequestDetailPage() {
  ...
  const getLeaveRequest = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/admin/pengajuan?id=${encodeURIComponent(requestId)}`,
        {
          method: "GET",
          cache: "no-store",
        },
  ...
  async function confirmUpdateLeaveStatus() {
    ...
    try {
      ...
      const response = await fetch("/api/admin/pengajuan", {
        method: "PATCH",
        headers: {
  ...
  return (
    <MobileShell variant="admin" withBottomPadding={false}>
      ...
      <AppHeader title="Detail Pengajuan" variant="admin" />
      ...
      <main className="min-h-dvh bg-gradient-to-br from-[#f6f8ff] via-white to-[#eef4ff]">
        <section className="mx-auto max-w-6xl space-y-5 px-5 py-6 md:px-10 lg:px-16">
          <button
            type="button"
            onClick={() => router.push("/admin/laporan-pengajuan")}
                    ...
                    {request.attachmentUrl ? (
                      <div className="mt-3 rounded-3xl bg-white p-4">
                        ...
                        <a
                          ...
                          className="mt-3 inline-flex items-center gap-2 rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm font-black text-[#123c8c] transition hover:bg-blue-100 active:scale-[0.98]"
                        >
                          <FileText size={17} strokeWidth={2.6} />
                          {request.attachmentName || "Buka Lampiran"}
                        </a>
                      </div>
                    ) : null}
