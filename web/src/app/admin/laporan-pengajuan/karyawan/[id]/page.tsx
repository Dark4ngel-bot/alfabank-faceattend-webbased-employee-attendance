...
export default function AdminEmployeeLeaveDetailPage() {
  ...
  const getEmployeeLeaveDetail = useCallback(async () => {
    try {
      ...
      const [employeesResponse, leaveResponse] = await Promise.all([
        ...
        fetch(
          `/api/admin/pengajuan?employeeId=${encodeURIComponent(employeeId)}`,
          {
            method: "GET",
            cache: "no-store",
          },
  ...
  return (
    <MobileShell variant="admin" withBottomPadding={false}>
      ...
      <AppHeader title="Detail Pengajuan Karyawan" variant="admin" />
                          ...
                          <div className="mt-3 space-y-2 border-t border-slate-100 pt-3">
                            ...
                            {request.attachmentUrl ? (
                              <a
                                ...
                                className="inline-flex w-fit items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-black text-[#123c8c] transition hover:bg-blue-100"
                              >
