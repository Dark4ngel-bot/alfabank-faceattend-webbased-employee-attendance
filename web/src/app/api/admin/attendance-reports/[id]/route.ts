...
function resolveCheckOutWorkMode(row: AttendanceDetailRow) {
  ...
  const activityNote = String(row.activityNote || "").toLowerCase();
  ...
  if (
    activityNote.includes("check-out") &&
    (activityNote.includes("wfh") || activityNote.includes("work from home"))
  ) {
    return "wfh";
  }
  ...
  if (
    activityNote.includes("check-out") &&
    (activityNote.includes("visit") || activityNote.includes("kunjungan"))
  ) {
    return "visit";
  }
  ...
  try {
    ...
    return NextResponse.json({
      ...
      report: {
        ...
        workModeLabel: formatWorkMode(primaryWorkMode),
