...
function mapLeaveRequest(item: {
  ...
  return {
    ...
    attachmentUrl: item.attachment_name
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("GET /api/admin/pengajuan error:", error);

    return jsonError(
      error instanceof Error
        ? error.message
        : "Gagal mengambil data pengajuan admin.",
      500
    );
  }
}
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (
      status === "approved" &&
      !["overtime", "lembur"].includes(existingRequest.leave_type)
    ) {
    ...
    console.error("PATCH /api/admin/pengajuan error:", error);
